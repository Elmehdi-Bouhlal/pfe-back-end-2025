...
return new class extends Migration {
    public function up()
    {
        Schema::create('books', function (Blueprint $table) {
            ...
            $table
                ->enum('language', ['english', 'french', 'arabic', 'spanish', 'other'])
                ->default('french')
                ->comment('Langue du livre');

            // Type de livre et condition
            $table
                ->enum('book_type', ['physical', 'digital'])
                ->default('physical')
                ->comment('Type: physique ou numérique');
            $table
                ->enum('book_condition', ['new', 'like-new', 'good', 'fair', 'poor'])
                ->nullable()
                ->comment('État du livre (pour livres physiques uniquement)');
            ...
            $table
                ->enum('status', ['draft', 'published', 'sold', 'removed', 'pending_approval'])
                ->default('draft')
                ->comment('Statut de la publication');
        ...
        Schema::table('books', function (Blueprint $table) {
            DB::statement('ALTER TABLE books ADD FULLTEXT idx_fulltext (title, author, description)');
        });
    }
